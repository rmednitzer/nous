...
- A comms link aging out is no longer silent (audit 2026-06-14 COMMS-2). When a
  link crosses `max_age_s` in `CommsSubsystem.step`, the connected-to-aged-out
  transition now stamps a cumulative `age_out_count` and a `last_aged_out_at_s`
  on the link, both surfaced through `comms_status`, and emits a structured
