...
| `src/nous/subsystems/eoir.py` | in-progress | EO/IR thermo-optical payload (BL-055 / ADR 0077): a per-band detection-range envelope `R0 x atm x signal x cal`, with a Koschmieder atmospheric cap, IR thermal-contrast crossover, EO night fall-off, and an RNG calibration drift; reads ambient live from the sensor pack; surfaced through `eoir_status`. |
