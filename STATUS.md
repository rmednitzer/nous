...
| `src/nous/engine.py` | in-progress | Tick orchestration; power, APU, thermal, compute, inference, storage, and comms subsystems wired through the tick loop. Comms aggregator drives `state.comms_state` each tick. |
...
| `src/nous/subsystems/{sensors,position,biometrics}.py` | planned | Typed stubs in v0.1. |
...
| `src/nous/estimators/{position,biometrics}.py` | planned | Typed stubs in v0.1. |
