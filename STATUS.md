...
| `src/nous/engine.py` | in-progress | Tick orchestration; power, APU, thermal, compute, inference, and storage subsystems wired through the tick loop. |
...
| `src/nous/subsystems/{sensors,position,biometrics,comms}.py` | planned | Typed stubs in v0.1. |
...
| `src/nous/estimators/{position,biometrics,comms}.py` | planned | Typed stubs in v0.1. |
