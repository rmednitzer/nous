...
def test_hand_crank_applies_efficiency() -> None:
    apu = ApuSubsystem(_profile())
    apu.set_hand_crank_w(10.0)
    apu.step(1.0)
    assert apu.truth()["hand_crank_w"] == pytest.approx(10.0 * 0.6)


def test_hand_crank_clamped_to_max() -> None:
    apu = ApuSubsystem(_profile())
    apu.set_hand_crank_w(100.0)
    apu.step(1.0)
    assert apu.truth()["hand_crank_w"] == pytest.approx(20.0 * 0.6)
...
def test_total_is_sum_of_sources() -> None:
    ...
    apu.step(1.0)
    total = apu.total_w
    assert total == pytest.approx(10.0 + 20.0 + 15.0 + 27.0 + 6.0)
