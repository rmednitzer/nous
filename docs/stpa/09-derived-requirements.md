...
| DR-7 | The OAuth issuer defaults to `single_client=true` and replaces a prior registration on re-DCR, so a second client cannot silently join. Disabling lockdown requires an ADR and emits a startup warning. | SC-7 | BL-019 | ADR-0008 | review |
