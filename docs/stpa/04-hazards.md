...
| H-2 | The state machine permits a transition into an operational mode (`MISSION`/`RELAY`/`MONITORING`/`C2`) while the reported thermal headroom is below the threshold. | L-2 |
...
| H-8 | The state machine permits (or sustains) a transition into an operational mode while the reported state-of-charge is below the critical reserve, stranding the device before it reaches a safe posture. | L-2 |
