...
| H-2 | The state machine permits a transition into an operational mode (`MISSION`/`RELAY`/`MONITORING`/`C2`) while the thermal estimator is above the headroom threshold. | L-2 |
...
| H-8 | The state machine permits (or sustains) a transition into an operational mode while the power estimator reports state-of-charge below the critical reserve, stranding the device before it reaches a safe posture. | L-2 |
