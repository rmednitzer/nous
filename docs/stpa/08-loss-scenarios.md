...
| LS-6 | A `MISSION` run drains the pack: the reported `soc_pct` crosses the profile's critical reserve while the controller, watching only the central endurance estimate, does not issue `low_power` in time (UCA-1c). The SC-8 entry gate (DR-11) would have refused re-entry, but the device is already operational; the per-tick auto-safing (DR-13) is what drives it to `LOW_POWER`. Were auto-safing not to fire (UCA-6a), the device would sustain the operational load below reserve and strand before reaching a safe posture. | UCA-1c, UCA-6a | L-2 |
