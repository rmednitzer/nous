...
| SC-2 | The state machine must refuse a transition into an operational mode (`MISSION`/`RELAY`/`MONITORING`/`C2`) when the reported thermal headroom is below the threshold (`headroom < threshold`). The gate reads the subsystem's reported state, not the estimator posterior; the threshold lives in the hardware profile. | H-2 |
...
| SC-8 | The state machine must refuse a transition into an operational mode (`MISSION`/`RELAY`/`MONITORING`/`C2`) when the reported `soc_pct` is below the critical reserve. The gate reads the subsystem's reported state, not the estimator posterior; the threshold lives in the hardware profile (`power.soc_pct_critical_threshold`) and reaches the gate as the `soc_pct_critical` safety-context signal. | H-8 |
