...
the 2026-05-20 baseline in [`AUDIT.md`](https://github.com/rmednitzer/nous/blob/main/AUDIT.md)
and the in-depth review in
[`docs/review-2026-05-21.md`](review-2026-05-21.md). It records
...
2. **N2 audit sink triage.** Restore audit JSONL writes on the live
   VM. Document the failure mode and root cause in `SECURITY.md`.
   Still the next live-VM action item.
...
Three of the original fourteen items are closed (N1, C3, C6). The
next single action with the largest leverage is **N2** (server-side),
followed by **C2** (required before BL-014 lands).

### 10.4 New (light) findings from this re-audit

These are opportunistic improvements rather than blockers; recording
them here so they are not lost.

- **N8 (Low). `docs/showcase/capability-matrix.md` deployment note
  was stale.** The note still referenced revision `02f2062` and the
  pre-catch-up drift. Updated in this PR to point at `43d0db2` and
  reference §10 of the audit.
- **N9 (Low). `test_tick_lifespan_stops_engine_when_tick_task_crashes`
  assertion is broader than its intent.** The test asserts that
  *some* exception in the surfaced `ExceptionGroup` is the simulated
  `RuntimeError`; an additional unrelated exception (e.g. from a
  fixture teardown) would still satisfy the assertion. The test's
  primary subject (the engine landing on `Mode.SHUTDOWN`) is asserted
  separately, so the impact is bounded; tighten by asserting
  `len(excinfo.value.exceptions) == 1` when the suite next touches
  that file (`tests/integration/test_server_lifespan.py:115-128`).
- **N10 (Low). AGENTS.md "Boundaries" and LIMITATIONS.md L17 did not
  yet reference `scripts/policy_checks.sh`.** Both surfaces talked
  about the standalone-repo rule without naming the enforcement seam.
  Updated in this PR.
