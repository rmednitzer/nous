...
| pmu / pdu | `parametric` | Bus regulation lifted off power: charge-limit clamp, CC/CV taper, dual-slot hot-swap arbitration (BL-005b, ADR 0075). No own estimator; the power Kalman follows the active pack across the bus. |
...
| eoir | `filtered` | Per-band EO/IR detection-range envelope (atmospheric, signal-contrast, calibration-drift) with terrain line-of-sight masking; two-channel gated Kalman (BL-055, ADR 0077/0078). |
| comms | `parametric` | Per-link envelopes drive FSM state each tick; per-link SIR particle filter (BL-030), a non-Gaussian belief rather than a Gaussian covariance bound. |
